...
function AvatarFallback({ className, ...props }: React.ComponentProps<typeof AvatarPrimitive.Fallback>) {
  return (
    <AvatarPrimitive.Fallback
      ...
      className={cn("flex size-full items-center justify-center rounded-full bg-[var(--ops-accent-soft)] text-xs font-semibold text-[var(--ops-accent-strong)]", className)}
      {...props}
    />
  );
}
