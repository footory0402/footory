...
export default function TeamPageClient({
  ...
  if (initialRole === "scout") {
    return (
      <div className="px-4 pt-4">
        <div className="flex flex-col items-center pt-16 text-center">
          <div className="mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-card text-4xl">🔍</div>
          ...
          <div className="mt-6 flex w-full max-w-[280px] flex-col gap-3">
            <Link
              href="/discover"
              className="rounded-full bg-accent py-3 text-center text-sm font-semibold text-bg"
            >
              팀 랭킹 보러가기
            </Link>
          </div>
  ...
  const currentTeams = teams.filter((team) => team.myRole !== "alumni");
  const alumniTeams = teams.filter((team) => team.myRole === "alumni");
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className="truncate text-sm font-semibold text-text-1">{team.name}</span>
          {team.myRole === "admin" && (
