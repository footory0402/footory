...
import type {
  ContactSettings,
  LinkedParent,
} from "@/app/profile/settings/page";
...
export default function SettingsPageClient({
  ...
  const [linkedParents, setLinkedParents] = useState(initialLinkedParents);
  ...
  const unlinkParent = useCallback(async (linkId: string, parentName: string) => {
    ...
    if (res.ok) {
      setLinkedParents((prev) => prev.filter((parent) => parent.linkId !== linkId));
