  ...
  return (
    <div className="pt-3 flex flex-col gap-4">
      ...
      {styleInfo && playStyle ? (
        ...
        >
          <div className="flex items-center gap-[10px]">
            <div
              ...
              style={{ width: 36, height: 36, borderRadius: 8, background: "rgba(250,204,21,0.06)", fontSize: 16 }}
            >
              {styleInfo.icon}
            </div>
            <div>
              <p className="m-0" style={{ fontFamily: "var(--font-body)", fontSize: 13, fontWeight: 700, color: "var(--color-text-1)" }}>
                {styleInfo.label}
              </p>
              <p className="m-0" style={{ fontFamily: "var(--font-body)", fontSize: 9, color: "var(--color-text-3)", marginTop: 1, fontStyle: "italic" }}>
          ...
          {onPlayStyleTest && (
            <button
              ...
              style={{
                padding: "6px 14px", borderRadius: 8, background: "rgba(212,168,83,0.08)",
                border: "1px solid rgba(212,168,83,0.20)", color: "var(--color-accent)",
        ...
        >
          <div className="flex items-center gap-[10px]">
            <div className="flex items-center justify-center" style={{ width: 36, height: 36, borderRadius: 8, background: "rgba(250,204,21,0.06)", fontSize: 16 }}>
              ⚡
            </div>
            <div>
              <p className="m-0" style={{ fontFamily: "var(--font-body)", fontSize: 13, fontWeight: 700, color: "var(--color-accent)" }}>
                플레이 스타일을 찾아보세요
              </p>
              <p className="m-0" style={{ fontFamily: "var(--font-body)", fontSize: 9, color: "var(--color-text-3)", marginTop: 1 }}>
      ...
      <div>
            ...
            onAddStat ? (
              <button
                ...
                style={{
                  ...
                  color: "var(--color-accent)", fontSize: 10, fontFamily: "var(--font-body)",
        ...
        {stats.length > 0 ? (
          <div className="grid grid-cols-2 gap-2">
            {stats.map((stat) => (
              <PhysicalTestCard
                key={stat.id}
                stat={stat}
                onTap={onViewHistory ? () => onViewHistory(stat.type) : undefined}
          ...
          <div className="card-elevated flex flex-col items-center gap-3 py-8 text-center">
            ...
            <p style={{ fontFamily: "var(--font-body)", fontSize: 13, fontWeight: 700, color: "var(--color-text-1)" }}>
              나의 성장을 기록해보세요
            </p>
            <p style={{ fontFamily: "var(--font-body)", fontSize: 11, color: "var(--color-text-3)", lineHeight: 1.5 }}>
            ...
            {onAddStat && (
              <button
                ...
                className="mt-1"
                style={{
                  padding: "8px 20px", borderRadius: 999, background: "var(--color-accent)",
                  color: "#000", fontSize: 12, fontWeight: 700, fontFamily: "var(--font-body)", cursor: "pointer",
                }}
              >
                첫 기록 시작하기
              </button>
        ...
        <div>
          ...
          <div className="card-elevated overflow-hidden">
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    borderBottom: i < growthStats.length - 1 ? "1px solid rgba(255,255,255,0.06)" : "none",
                  }}
                >
                  <span style={{ fontFamily: "var(--font-body)", fontSize: 12, color: "var(--color-text-2)" }}>
                    {meta.label}
                  </span>
                  <div className="flex items-center gap-2">
                    <span style={{ fontFamily: "var(--font-stat)", fontSize: 14, fontWeight: 600, color: "var(--color-text-1)" }}>
                    ...
                    {diff > 0 && (
                      <span
                        style={{
                          fontSize: 11, fontFamily: "var(--font-stat)", fontWeight: 600,
            ...
            <div style={{ padding: "6px 14px", textAlign: "center" }}>
              <span style={{ fontSize: 9, color: "var(--color-text-3)", fontFamily: "var(--font-body)" }}>
                첫 기록 대비 변화량
              </span>
            </div>
          </div>
...
function PhysicalTestCard({
  ...
  const improved = diff != null && diff !== 0 && (meta.lowerIsBetter ? diff < 0 : diff > 0);

  const isTeam = stat.verified;
  ...
  const measuredDate = stat.measuredAt
    ? (() => { const d = new Date(stat.measuredAt); return `${d.getFullYear()}.${d.getMonth() + 1}.${d.getDate()}`; })()
    : "";
  ...
  return (
    <div
      ...
      style={{
        ...
        ...(isTeam && { borderColor: "rgba(74,222,128,0.18)", backgroundColor: "rgba(74,222,128,0.04)" }),
      }}
      onClick={onTap}
    ...
    >
      ...
      <div className="mb-[6px] flex items-center justify-between">
        <span style={{ fontFamily: "var(--font-body)", fontSize: 11, fontWeight: 600, color: "var(--color-text-2)" }}>
          {meta.label}
        </span>
        <span style={{ fontSize: 9, color: "var(--color-text-3)", fontFamily: "var(--font-body)" }}>
          {measuredDate}
      ...
      <div className="mb-2 flex items-baseline gap-[3px]">
        <span style={{ fontFamily: "var(--font-stat)", fontSize: 28, fontWeight: 700, color: "var(--color-text-1)", lineHeight: 1 }}>
          {formatStatValue(stat.value, stat.type, stat.unit)}
        </span>
        <span style={{ fontFamily: "var(--font-body)", fontSize: 10, color: "var(--color-text-3)" }}>
          {!isTime && displayUnit}
        </span>
        {diff != null && diff !== 0 && (
          <span className="ml-[2px]" style={{ fontSize: 10, fontFamily: "var(--font-stat)", color: improved ? "var(--color-green)" : "var(--color-red)" }}>
            {improved ? "↑" : "↓"}
      ...
      {/* 퍼센타일 섹션 */}
      {percentileLoading ? (
        <div className="animate-pulse rounded-full" style={{ height: 7, background: "rgba(255,255,255,0.08)", marginTop: 8 }} />
      ) : peerCount != null && peerCount < 3 ? (
        <p style={{ fontFamily: "var(--font-body)", fontSize: 9, color: "var(--color-text-3)", marginTop: 8, marginBottom: 0 }}>데이터 수집 중</p>
      ) : showPercentile ? (
        <div style={{ marginTop: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <div style={{ flex: 1, position: "relative", height: 7, borderRadius: 4, background: "rgba(255,255,255,0.08)", overflow: "visible" }}>
              <div
                style={{
                  height: "100%", borderRadius: 4, width: `${percentile}%`,
            ...
            {tier ? (
              <span
                style={{
                  flexShrink: 0, borderRadius: 999, padding: "2px 7px", fontSize: 9, fontWeight: 700,
                  fontFamily: "var(--font-body)",
                  background: tier.color === "#D4A853" ? "rgba(212,168,83,0.15)" : "rgba(161,161,170,0.10)",
                  color: tier.color,
                  border: tier.color === "#D4A853" ? "1px solid rgba(212,168,83,0.3)" : "1px solid rgba(161,161,170,0.2)",
                  whiteSpace: "nowrap",
                }}
              >
              ...
              <span style={{ flexShrink: 0, fontSize: 9, fontFamily: "var(--font-body)", color: "#4ADE80" }}>성장 중</span>
            )}
          </div>
          <p style={{ fontFamily: "var(--font-body)", fontSize: 9, color: "var(--color-text-3)", marginTop: 4, marginBottom: 0 }}>
            {ageAvg != null && (<>동나이 평균 {formatStatValue(ageAvg, stat.type, stat.unit)}{!isTime && displayUnit} · </>)}
      ...
      {/* 탭하여 기록 보기 힌트 */}
      {onTap && (
        <p style={{ fontFamily: "var(--font-body)", fontSize: 9, color: "rgba(255,255,255,0.20)", textAlign: "right", marginTop: 4, marginBottom: 0, letterSpacing: "-0.01em" }}>
          탭하여 기록 보기
        </p>
      )}
    </div>
