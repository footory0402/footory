...
- profile card 표시
- lower third 표시 제어
- 확대 재생은 `주인공 선택` 뒤에만 필요할 때 선택적으로 조정한다.
- 편집 화면의 기본 구조는 `영상 + 하단 미니바 + 저장` 하나로 유지한다.
...
### 4. 선택 편집 화면
- 목적: 필요한 경우에만 주인공, trim, profile card, lower third를 한 화면에서 조정
- 포함 요소: 프리뷰, 하단 미니바, 시작/끝 조정, 원하는 시점의 주인공 선택, profile card 토글, lower third 토글, 저장 CTA

## 확대 재생 정책
- 확대 재생은 독립 편집 기능이 아니라 `주인공 강조`의 일부다.
- 사용자는 먼저 영상에서 주인공을 한 번 누르고, 필요할 때만 확대 정도를 간단히 조정한다.
- 첫 선택만으로 자동 확대를 걸지 않고, 기본 구도는 `원본`으로 유지한다.
- 확대 조정은 선수 선택 뒤에만 짧은 칩으로 제공하고, 필요하면 핀치로만 미세 조정한다.
...
- 편집 화면에서의 간단한 확대 미리보기와 핀치 미세 조정
...
- 장면, 주인공, 카드, 하단 정보는 별도 탭으로 나누지 않고 한 화면의 하단 미니바에서 끝내야 한다.
- 중앙 재생 오버레이, `재생`/`구간` 같은 설명성 라벨, 장문 도움말은 기본 편집 화면에 두지 않는다.
