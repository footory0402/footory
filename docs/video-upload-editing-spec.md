...
- 주인공 선택, 구간 자르기, profile card, lower third는 쉽게 접근 가능한 기본 편집 도구로 유지한다.
...
- 포함 요소: 프리뷰, 주인공 선택, 시작/끝 조정, profile card 토글, lower third 토글

## 확대 재생 정책
- 확대 재생은 독립 편집 기능이 아니라 `주인공 강조`의 일부다.
- 사용자는 먼저 영상에서 주인공을 한 번 누르고, 필요할 때만 확대 정도를 간단히 조정한다.
- 기본 확대값은 바로 재생해도 어색하지 않은 중간값을 사용한다.
